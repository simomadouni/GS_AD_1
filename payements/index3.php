    ...
    <section id="content">
        ...
        <nav>
            ...
            if (isset($_POST['submit'])) {
                ...
                $sql = "SELECT * FROM payments WHERE AmountPaid LIKE '%$search%'";
                                ...
                                <div class="table-data">
                                    <div class="order">
                                        <div class="head">
                                            <h3>LISTE</h3>
                                            <h4>
                                                <a href="payment-create.php" class="btn btn-primary float-end">Add Payement</a>
                                            </h4>
                                        </div>
                                        <table>
                                            <thead>
                                            ...
                                            <tbody>
                                                <?php
                                                $query = "SELECT payments.id, students.FirstName, payments.AmountPaid, payments.PaymentDate, payments.PaymentMethod
                                                        FROM payments
                                                        LEFT JOIN students ON payments.students_id = students.id";

                                                if (isset($_POST['submit'])) {
                                                    $search = $_POST['search'];
                                                    // Modify the SQL query to search for payments with a specific amount
                                                    $sql = "SELECT payments.id, students.FirstName, payments.AmountPaid, payments.PaymentDate, payments.PaymentMethod
                                                        FROM payments
                                                        LEFT JOIN students ON payments.students_id = students.id
                                                        WHERE payments.AmountPaid LIKE '%$search%'";
                                                    $query = $sql;
                                                }
                                                ...
                                                if (mysqli_num_rows($query_run) > 0) {
                                                    foreach ($query_run as $payments) {
                                                ?>
                                                        <tr>
                                                            <td><?= $payments['id']; ?></td>
                                                            <td>
                                                                <?php
                                                                // Check if 'FirstName' key is set before accessing it
                                                                echo isset($payments['FirstName']) ? $payments['FirstName'] : "N/A";
                                                                ?>
                                                            </td>
                                                            <td><?= $payments['AmountPaid'];
                                                                echo " DH" ?></td>
                                                            <td><?= $payments['PaymentDate']; ?></td>
                                                            <td><?= $payments['PaymentMethod']; ?></td>
                                                            <td>
                                                                <a href="payement-edit.php?id=<?php echo $payments['id']; ?>" class="btn btn-warning btn-sm ">Edit</a>
                                                                <form action="code3.php" method="POST" class="d-inline">
                                                                    <input type="hidden" name="id" value="<?php echo $payments['id']; ?>">
